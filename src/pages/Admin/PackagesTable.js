...
function PackagesDashboard() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = isEditing
        ? `https://spa-backend-gc9u.onrender.com/api/packages/${id}`
        : "https://spa-backend-gc9u.onrender.com/api/packages";
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`https://spa-backend-gc9u.onrender.com/api/packages/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
          ...
          {packages.map((p) => (
            <tr key={p.id} className="hover:bg-gray-50">
              <td className="p-2 border">
                {p.image ? (
                  <img
                    src={`https://spa-backend-gc9u.onrender.com/${p.image}`}
                    alt={p.title}
