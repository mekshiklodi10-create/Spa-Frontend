...
function ServicesDashboard() {
  ...
  const fetchServices = async () => {
    try {
      ...
      const res = await fetch("https://spa-backend-2tfg.onrender.com/api/services", {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = isEditing
        ? `https://spa-backend-2tfg.onrender.com/api/services/${id}`
        : `https://spa-backend-2tfg.onrender.com/api/services`;
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      const res = await fetch(`https://spa-backend-2tfg.onrender.com/api/services/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
          ...
          {services.map((s) => (
            <tr key={s.id} className="hover:bg-gray-50">
              <td className="p-2 border">
                {s.image ? (
                  <img
                    src={`https://spa-backend-2tfg.onrender.com${s.image}`}
                    alt={s.title}
